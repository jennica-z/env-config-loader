...
import envloader
from envloader.cli import cli as dotenv_cli
from envloader.version import __version__
...
def test_list_no_file(cli):
    result = cli.invoke(envloader.cli.list_values, [])
...
def test_set_non_existent_file(cli):
    result = cli.invoke(envloader.cli.set_value, ["a", "b"])
