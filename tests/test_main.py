...
def test_set_key_no_file(tmp_path):
    nx_path = tmp_path / "nx"
    logger = logging.getLogger("envloader.main")

    with mock.patch.object(logger, "warning"):
        result = envloader.set_key(nx_path, "foo", "bar")
...
def test_set_key(dotenv_path, before, key, value, expected, after):
    ...
    with mock.patch.object(logger, "warning") as mock_warning:
        result = envloader.set_key(dotenv_path, key, value)
...
def test_set_key_encoding(dotenv_path):
    ...
    result = envloader.set_key(dotenv_path, "a", "é", encoding=encoding)
...
def test_set_key_preserves_file_mode(dotenv_path):
    ...
    envloader.set_key(dotenv_path, "a", "y")
...
def test_rewrite_closes_file_handle_on_lstat_failure(tmp_path):
    ...
    with mock.patch("envloader.main.os.lstat", side_effect=FileNotFoundError):
        with mock.patch("envloader.main.open", side_effect=tracking_open):
            envloader.set_key(dotenv_path, "a", "x")
...
def test_set_key_follow_symlinks(tmp_path):
    ...
    envloader.set_key(symlink, "a", "y", follow_symlinks=True)
...
def test_set_key_permission_error(dotenv_path):
    ...
    with pytest.raises(PermissionError):
        envloader.set_key(dotenv_path, "a", "b")
...
def test_get_key_no_file(tmp_path):
    ...
    with (
        ...
        result = envloader.get_key(nx_path, "foo")

    assert result is None
    mock_info.assert_has_calls(
        calls=[
            mock.call("env-config-loader could not find configuration file %s.", nx_path)
        ],
    )
...
def test_get_key_not_found(dotenv_path):
    ...
    with mock.patch.object(logger, "warning") as mock_warning:
        result = envloader.get_key(dotenv_path, "foo")
...
def test_get_key_encoding(dotenv_path):
    ...
    result = envloader.get_key(dotenv_path, "é", encoding=encoding)
...
def test_unset_with_value(dotenv_path):
    ...
    with mock.patch.object(logger, "warning") as mock_warning:
        result = envloader.unset_key(dotenv_path, "a")
...
def test_unset_no_value(dotenv_path):
    ...
    with mock.patch.object(logger, "warning") as mock_warning:
        result = envloader.unset_key(dotenv_path, "foo")
...
def test_unset_encoding(dotenv_path):
    ...
    result = envloader.unset_key(dotenv_path, "é", encoding=encoding)
...
def test_unset_non_existent_file(tmp_path):
    ...
    with mock.patch.object(logger, "warning") as mock_warning:
        result = envloader.unset_key(nx_path, "foo")
...
def test_unset_key_symlink_to_missing_file(tmp_path):
    ...
    with mock.patch.object(logger, "warning") as mock_warning:
        result = envloader.unset_key(symlink, "a")
...
def test_unset_key_follow_symlinks(tmp_path):
    ...
    envloader.unset_key(symlink, "a", follow_symlinks=True)
...
def test_find_dotenv_no_file_raise(tmp_path):
    ...
    with pytest.raises(IOError):
        envloader.find_dotenv(raise_error_if_not_found=True, usecwd=True)
...
def test_find_dotenv_no_file_no_raise(tmp_path):
    ...
    result = envloader.find_dotenv(usecwd=True)
...
def test_load_config_from_env_file_existing_file(dotenv_path):
    dotenv_path.write_text("a=b")

    result = envloader.load_config_from_env_file(dotenv_path)
...
def test_load_config_from_env_file_disabled(dotenv_path, flag_value):
...
def test_load_config_from_env_file_disabled_notification(dotenv_path, flag_value):
    with mock.patch.dict(
        os.environ, {"PYTHON_DOTENV_DISABLED": flag_value}, clear=True
    ):
        dotenv_path.write_text("a=b")
        ...
        mock_debug.assert_called_once_with(
            "env-config-loader: .env loading disabled by PYTHON_DOTENV_DISABLED environment variable"
        )
...
def test_load_config_from_env_file_enabled(dotenv_path, flag_value):
...
def test_load_config_from_env_file_enabled_no_notification(dotenv_path, flag_value):
    with mock.patch.dict(
        os.environ, {"PYTHON_DOTENV_DISABLED": flag_value}, clear=True
    ):
        dotenv_path.write_text("a=b")
...
@mock.patch.dict(os.environ, {}, clear=True)
def test_load_config_from_env_file_doesnt_disable_itself(dotenv_path):
...
def test_load_config_from_env_file_no_file_verbose():
    logger = logging.getLogger("envloader.main")

    with mock.patch.object(logger, "info") as mock_info:
        result = envloader.load_config_from_env_file(".does_not_exist", verbose=True)

    assert result is False
    mock_info.assert_called_once_with(
        "env-config-loader could not find configuration file %s.", ".does_not_exist"
    )
...
def test_load_config_from_env_file_existing_variable_no_override(dotenv_path):
    dotenv_path.write_text("a=b")

    result = envloader.load_config_from_env_file(dotenv_path, override=False)
...
@mock.patch.dict(os.environ, {"a": "c"}, clear=True)
def test_load_config_from_env_file_existing_variable_override(dotenv_path):
    dotenv_path.write_text("a=b")

    result = envloader.load_config_from_env_file(dotenv_path, override=True)
...
@mock.patch.dict(os.environ, {"a": "c"}, clear=True)
def test_load_config_from_env_file_redefine_var_used_in_file_no_override(dotenv_path):
    dotenv_path.write_text('a=b\nd="${a}"')
...
@mock.patch.dict(os.environ, {"a": "c"}, clear=True)
def test_load_config_from_env_file_redefine_var_used_in_file_with_override(dotenv_path):
    dotenv_path.write_text('a=b\nd="${a}"')
...
@mock.patch.dict(os.environ, {}, clear=True)
def test_load_config_from_env_file_string_io_utf_8():
    stream = io.StringIO("a=à")

    result = envloader.load_config_from_env_file(stream=stream)
...
@mock.patch.dict(os.environ, {}, clear=True)
def test_load_config_from_env_file_file_stream(dotenv_path):
    dotenv_path.write_text("a=b")

    with dotenv_path.open() as f:
        result = envloader.load_config_from_env_file(stream=f)

    assert result is True
    assert os.environ == {"a": "b"}


def test_load_config_from_env_file_in_current_dir(tmp_path):
    dotenv_path = tmp_path / ".env"
    ...
    code_path.write_text(
        ...
        envloader.load_config_from_env_file(verbose=True)
        print(os.environ['a'])
    """)
    )
    os.chdir(tmp_path)
...
def test_dotenv_values_file(dotenv_path):
    ...
    result = envloader.dotenv_values(dotenv_path)
...
def test_dotenv_values_string_io(env, string, interpolate, expected):
    with mock.patch.dict(os.environ, env, clear=True):
        ...
        result = envloader.dotenv_values(stream=stream, interpolate=interpolate)
...
def test_dotenv_values_file_stream(dotenv_path):
    ...
    with dotenv_path.open() as f:
        result = envloader.dotenv_values(stream=f)
