...
def _load_config_from_env_file_disabled() -> bool:
    """
...
def with_warn_for_invalid_lines(mappings: Iterator[Binding]) -> Iterator[Binding]:
    for mapping in mappings:
        if mapping.error:
            logger.warning(
                "env-config-loader could not parse statement starting at line %s",
...
class EnvConfig:
    ...
    def _get_stream(self) -> Iterator[IO[str]]:
        if self.dotenv_path and _is_file_or_fifo(self.dotenv_path):
            ...
            if self.verbose:
                logger.info(
                    "env-config-loader could not find configuration file %s.",
...
def get_key(
    ...
    """
    return EnvConfig(dotenv_path, verbose=True, encoding=encoding).get(key_to_get)
...
def load_config_from_env_file(
    ...
    """
    if _load_config_from_env_file_disabled():
        logger.debug(
